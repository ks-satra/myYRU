...
<section class="content">
    <div class="box box-danger">
        <div class="box-header">
            <h3 class="box-title"> ข้อมูลการแจกจ่ายหนังสือ</h3> > ชุดหนังสือรวมทั้งหมด
            <div style="margin-top: 15px;">
                <form action="./" method="get" class="form-inline">
                    <input type="hidden" name="content" value="book-get-person">
                    <input type="hidden" name="page" value="<?php echo $PAGE; ?>">
                    <a href="?content=book-distribute-add" class="btn btn-success" title="เพิ่มข้อมูล">
                        <i class="fa fa-plus"></i> เพิ่มข้อมูล</a>
                    <button type="button" class="btn btn-default" onClick="location.href='?content=book-get-person'" title="รีเฟรส"><i class="fa fa-refresh"></i> รีเฟรส</button>
        ...
        <div class="box-body" style="margin-top: 0px;">
            <div class="table-responsive">
                        ...
                        if(sizeof($DATA)>0){
                            foreach($DATA as $key => $row){
                                ...
                                <tr>
                                    ...
                                    <td title="<?php echo $row['school_name'];?> ต.<?php echo $row['district_name'];?> อ.<?php echo $row['amphur_name'];?> จ.<?php echo $row['province_name'];?>"><span style="text-overflow: ellipsis;overflow: hidden;white-space: nowrap; display: inline-block;width:220px"><?php echo $row['school_name'];?> ต.<?php echo $row['district_name'];?> อ.<?php echo $row['amphur_name'];?> จ.<?php echo $row['province_name'];?></span></td>
                                    <td style="margin-right: 3px; text-align: right;">
                                        <a class="btn btn-info btn" href="?content=book-get-show&id=<?php echo $row['teacher_id'];?>&school_id=<?php echo $row['school_id'];?>&page=<?php echo $PAGE; ?>" title="ตรวจสอบข้อมูล"><i class="fa fa-eye"></i>
                                        </a>
                                    </td>             
                                </tr>
                                <?php 
                ...
                <?php
                    ...
                    $disabled1 = '';
                    $disabled2 = '';
                    $href1 = 'href="?content=book-get-person&page='.($PAGE-1).$searhing.'"';
                    $href2 = 'href="?content=book-get-person&page='.($PAGE+1).$searhing.'"';
