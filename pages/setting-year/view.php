...
<section class="content-header">
    <h1>ตั้งค่าข้อมูลปี<small>การจัดการข้อมูลปี</small></h1>
    <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> การจัดการข้อมูลปี</a></li>
    </ol>
</section>
<section class="content">
    <div class="box box-danger">
        <div class="box-header">
          	<h3 class="box-title">การจัดการข้อมูลปี</h3>
        </div>
        <div class="box-body"> 
            <a href="?content=criterion-add" class="btn btn-default" title="กลับ"><i class="fa fa-arrow-left"></i> กลับ</a>
        	<script>
        		var action = "";
        	...
        	<?php
        		...
        		function showAlert($message) {
        			echo '
        				<script>
        					alert("'.$message.'");
        					location.href="./?content=setting-year";
        				</script>
        			';
        		}

        		if( isset($_POST["btnAdd"]) ) {
        			$id = $DATABASE->QueryMaxId("tb_year","id");
        			$name = $_POST["name"];
        			$sql = "
						INSERT INTO tb_year (
							id,
        		...
        		if( isset($_POST["btnDel"]) ) {
        			$id = $_POST["id"];
        			$sql = "
						DELETE FROM tb_year WHERE id='".$id."'
        			";
        			$message = "";
        			if( $DATABASE->Query($sql) ) {
        	...
        	?>
        	<form class="form-horizontal" action="./?content=setting-year" method="POST" onsubmit="return onSubmit();">
			  	<div class="form-group">
			    	<label for="id" class="col-sm-2 control-label">รหัสปี</label>
			    	<div class="col-sm-6">
			      		<input type="text" class="form-control" id="id" name="id" placeholder="รหัสปี" readonly="readonly">
			    	</div>
			  	</div>
			  	<div class="form-group">
			    	<label for="name" class="col-sm-2 control-label">ชื่อปี</label>
			    	<div class="col-sm-6">
			      		<input type="text" class="form-control" id="name" name="name" placeholder="ชื่อปี" required="required" maxlength="4">
			    	</div>
			  	</div>
			  	<div class="form-group">
	        ...
	        <tbody>
	        	<?php
	        		$sql = "SELECT * FROM tb_year ORDER BY id";
