                    ...
                    <tbody>
                        ...
                        if(sizeof($DATA)>0){
                            foreach($DATA as $key => $row){
                                ?>
                                <?php 
                                    // $qty_card = $DATABASE->QueryString("SELECT COUNT(card) FROM tb_person WHERE card ='".$row['id']."'");
                                    // $qty_participant = $DATABASE->QueryString("SELECT COUNT(activity_id) FROM tb_person WHERE activity_id ='".$row['id']."' AND project_id = '".$_GET['project_id']."' AND card = '".$."'");
                                ?>
                                <tr>
                                    <td><center><?php echo $key+1; ?></center></td>
                                    <td><?php echo $row['name']; ?></td>
                                    <td>
                                        <a class="btn btn-success btn-sm" href="?content=project-activity-class-add&project_id=<?php echo $PROJECT_ID;?>&activity_id=<?php echo $row['id'];?>&page=<?php echo $PAGE; ?>" title="เพิ่มผู้เข้าร่วม"><i class="fa fa-plus"></i> เพิ่มผู้เข้าร่วม <span class="badge badge-light"><?php //echo $qty_participant;?></span></a>
                                    </td>             
                                </tr>
                                <?php 
                            }
